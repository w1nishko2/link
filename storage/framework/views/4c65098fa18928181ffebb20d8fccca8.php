...
document.addEventListener('DOMContentLoaded', function() {
    // Загружаем данные услуги в интерфейс
    loadServiceData();
...
// Загрузка данных услуги в интерфейс
function loadServiceData() {
    // Загружаем основные данные
    document.querySelector('.editable-title').textContent = formState.title;
    document.querySelector('.editable-description').textContent = formState.description;
    document.querySelector('.editable-button').textContent = formState.buttonText;
    
    // Настраиваем цену
    if (formState.hasPrice && formState.price) {
        document.querySelector('.service-price').style.display = 'block';
        document.querySelector('.editable-price').textContent = formState.price;
        document.getElementById('add-price-card-btn').style.display = 'none';
    } else {
        document.querySelector('.service-price').style.display = 'none';
        document.getElementById('add-price-card-btn').style.display = 'inline-block';
    }
    
    // Настраиваем дополнительные поля
    document.getElementById('price-type-select').value = formState.priceType;
    document.getElementById('order-input').value = formState.orderIndex;
}
...
function initializeSwiper() {
    ...
    swiperInstance = new Swiper('.swiper.services-swiper', {
        slidesPerView: 1,
        spaceBetween: 20,
        grabCursor: true,
        centeredSlides: true,
...
function bindEvents() {
    ...
    descriptionElement.addEventListener('blur', function() {
        if (this.textContent.trim() === '') {
            this.textContent = 'Описание услуги';
...
function saveService() {
    ...
    if (!formState.description || formState.description === 'Описание услуги') {
