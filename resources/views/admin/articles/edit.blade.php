...
<div class="row">
                ...
                <form action="{{ route('admin.articles.update', [$currentUserId, $article]) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('excerpt') is-invalid @enderror" 
                                  id="excerpt" name="excerpt" rows="3" required maxlength="300">{{ old('excerpt', $article->excerpt) }}</textarea>
                        @error('excerpt')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-3">
                        ...
                        @if($article->image_path)
                            <div class="mb-2">
                                <img src="{{ asset('storage/' . $article->image_path) }}" class="img-thumbnail" style="max-height: 100px;" alt="Текущее изображение">
                        ...
                        @error('image')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <div class="form-text">Поддерживаются изображения в любых форматах. Максимальный размер: 10MB. Все изображения автоматически конвертируются в WebP с оптимизацией размера. Оставьте пустым, чтобы сохранить текущее изображение.</div>
                    </div>
    ...
    <div class="col-lg-4">
        <div class="card">
            ...
            <div class="card-body">
                <div id="preview">
                    <h6 class="preview-title">{{ $article->title }}</h6>
                    <p class="preview-excerpt">{{ $article->excerpt }}</p>
                    @if($article->image_path)
                        <div class="preview-image">
                            <img src="{{ asset('storage/' . $article->image_path) }}" style="width: 100%; height: 120px; object-fit: cover; border-radius: 4px;" alt="Preview">
                        </div>
                    @else
    ...
    imageInput.addEventListener('change', function() {
        ...
        if (file) {
            ...
            reader.onload = function(e) {
                previewImage.innerHTML = `<img src="${e.target.result}" style="width: 100%; height: 120px; object-fit: cover; border-radius: 4px;" alt="Preview">`;
            };
