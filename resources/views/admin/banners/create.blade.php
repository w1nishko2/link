...
<form id="banner-form" action="{{ route('admin.banners.store', $currentUserId) }}" method="POST" enctype="multipart/form-data" style="display: none;">
    @csrf
...
function loadOldValues() {
    ...
    @if(old('link_url') && old('link_text'))
        formState.linkUrl = "{{ old('link_url') }}";
        formState.linkText = "{{ old('link_text') }}";
        formState.hasLink = true;
        document.querySelector('.editable-link-text').textContent = "{{ old('link_text') }}";
        updateLinkDisplay();
    @endif
